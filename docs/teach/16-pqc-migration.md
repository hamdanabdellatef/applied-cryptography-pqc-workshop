# Teach Session 16: PQC migration
...
| --- | --- | --- |
| 0–15 | Plan dependencies before deadlines. List long-lived confidentiality flows separately from signature trust. Ask which passive recordings remain exposed while a legacy reader blocks migration. | Learners label the relevant assets and authority. |
| 15–36 | Run the dependency graph and explain the twelve-month modeled finish. Change one task duration, rerun downstream calculation, and discuss parallel work. The invented durations are not a forecast. Pause before execution for a written prediction. | A prediction, observed result and explanation of any difference. |
| 36–50 | Pair exercise: A new verifier accepts PQ signatures, but its update package is bootstrapped using an untrusted key. Require an authenticated trust-distribution path and staged acceptance-policy transition. | One annotated boundary diagram and one explicit residual risk. |
...
**Discussion question:** Why can replacing a cryptographic library leave migration incomplete?

Expected reasoning: Protocols, formats, trust anchors, peers, deployment policy, retained data and recovery tools may still depend on the old scheme.
...
Have pairs complete the relevant rows in the [review worksheet](../resources/migration-worksheet.md). One learner proposes a design and the other plays the attacker. Switch roles halfway through. Require each pair to write an assumption that, if false, invalidates its strongest claim.
...
1. Why can replacing a cryptographic library leave migration incomplete?
